...
def pre_processing_data(process_id, folder_list, downsampling, network_downsampling, is_hsv, inlier_percentage,
    ...
    for folder in folder_list:
        ...
        if len(undistorted_camera_intrinsic_per_view)==1:
            for i in range(len(visible_view_indexes)-1):
               undistorted_camera_intrinsic_per_view.append(undistorted_camera_intrinsic_per_view[0]) 
        print('len(undistorted_camera_intrinsic_per_view={}'.format(len(undistorted_camera_intrinsic_per_view)))
        ...
        if len(point_cloud[0])>4:
            for i in range(0,len(point_cloud)):
                del point_cloud[i][3:6:1]

...
class DepthScalingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("input_sparse_binary_masks:{}".format(input_sparse_binary_masks.shape))
        #print("mean_sparse_depths:{}".format(mean_sparse_depths.shape))
        #print("above_mean_masks:{}".format(above_mean_masks.shape))
        ...
        #print("sparse_scale_maps:{}".format(sparse_scale_maps.shape))
        #print("mean_scales:{}".format(mean_scales.shape))
        #print("centered_sparse_scale_maps:{}".format(centered_sparse_scale_maps.shape))
        #print("scale_stds:{}".format(scale_stds))
...
def _warp_coordinate_generate(depth_maps_1, img_masks, translation_vectors, rotation_matrices, intrinsic_matrices):
    ...
    #print("rotation_matrices{}:".format(rotation_matrices.shape))
    #print("rotation_matrices{}:".format(rotation_matrices))
    #print("rotation_matrices_inverse{}".format(rotation_matrices_inverse))
    #print("rotation_matrices_inverse{}".format(rotation_matrices_inverse.shape))
    ...
    #print("intrinsic product with rotation{}:".format(temp_mat))
    W = torch.bmm(temp_mat, -translation_vectors)
    #print("intrinsic product with rotation and negative of translation{}:".format(W))
    M = torch.bmm(temp_mat, intrinsic_matrices_inverse)
    #print("product of intrinsic, rotation and intrinsic inverse{}:".format(M))
    ...
    #print("M size:{}".format((M.view(-1, 1, 1, 3, 3)).shape))
    #print("shape of mesh_grid:{}".format(mesh_grid.shape))
    # p=torch.matmul(M.view(-1, 1, 1, 3, 3), mesh_grid)
    ...
    depth_maps_2_calculate = W.view(-1, 3).narrow(dim=-1, start=2, length=1).view(-1, 1, 1, 1) + torch.mul(
        ...
        intermediate_result.narrow(dim=-1, start=2, length=1).view(-1, height,
                                                                   width, 1))
    #print("W.view(-1, 3).narrow(dim=-1, start=2, length=1):{}".format((W.view(-1, 3).narrow(dim=-1, start=2, length=1)).shape))
    #print("W.view(-1, 3).narrow(dim=-1, start=2, length=1).view(-1, 1, 1, 1):{}".format(W.view(-1, 3).narrow(dim=-1, start=2, length=1).view(-1, 1, 1, 1).shape))
    #print("intermediate_result.narrow(dim=-1, start=2, length=1):{}".format((intermediate_result.narrow(dim=-1, start=2, length=1)).shape))
    #print("depth_maps_2_calculate:{}".format(depth_maps_2_calculate.shape))
...
def _flow_from_depth(depth_maps_1, img_masks, translation_vectors, rotation_matrices, intrinsic_matrices):
    ...
    #print("depth_maps_1:{}".format(depth_maps_1.shape))
